...
class Metrics(object):
    def __init__(self, options, clients, name=''):
        ...
        self.participation_history = {c.id: [] for c in clients}
    ...
    def write(self):
        ...
        x = len(self.loss_on_g_test_data)  # 这里简单假设只有一个数据点，如果有多个loss和acc值，需按实际修改
        y = len(self.acc_on_g_test_data)
        loss_y = [self.loss_on_g_test_data]
        acc_y = [self.acc_on_g_test_data]

        plt.figure(figsize=(10, 5))
        plt.subplot(1, 2, 1)
        plt.plot(x, loss_y, marker='o')
        plt.title('Loss on Global Test Data')
        plt.xlabel('Epoch (assuming single point here, adjust as needed)')
        plt.ylabel('Loss')

        plt.subplot(1, 2, 2)
        plt.plot(y, acc_y, marker='*')
        plt.title('Accuracy on Global Test Data')
        plt.xlabel('Epoch (assuming single point here, adjust as needed)')
        plt.ylabel('Accuracy')

        plot_dir = os.path.join(self.result_path, self.exp_name, 'plots')
        print(plot_dir)
        if not os.path.exists(plot_dir):
            os.makedirs(plot_dir)
        plot_path = os.path.join(plot_dir, 'loss_acc_plot.png')
